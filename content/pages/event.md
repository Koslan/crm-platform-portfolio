...
-   Book a meeting and it appears in the Meetings tab, marked _not in calendar_ — because sending the invitation is a server-side step, and the screen says so rather than pretending.
...
That is also what makes the ceilings unavoidable rather than incidental. The same query ceiling and the same write validation apply here as in production — so the target list pages at two hundred rows, the wizard genuinely refuses an incomplete booking, and a meeting that has not reached a calendar says so instead of pretending. A screen that hides its platform is a screenshot; this one has to survive it.
