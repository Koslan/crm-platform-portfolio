...
await p.waitForTimeout(200);
say('full view adds columns', (await p.locator('#c-tw thead tr').first().locator('th').count())>=13);
await p.locator('#c-tw thead tr.fr input').first().fill('yes');
await p.waitForTimeout(300);
say('column filter narrows the table', /of \d+ rows/.test(await p.locator('#c-tw .tfoot').textContent()));
await p.locator('#c-tw [data-clear]').click(); await p.waitForTimeout(200);
